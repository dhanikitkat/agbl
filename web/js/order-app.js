(function () {
  ...
  function customerTypeLabel() {
    return els.customerType.value === 'perorangan'
      ? 'Perorangan'
      : 'Pabrik ' + els.pabrikName.value.trim();
  }
  ...
  function saveOrder() {
    ...
    AGBL.api('createOrder', payload).then(function (res) {
      ...
      // Keep same idempotency if user retries immediately after success? new key next time
    }).catch(function (err) {
      AGBL.enqueueOrder(payload);
  ...
  function printNota() {
    ...
    var html = '<html><head><title>Nota ' + escapeHtml(settings.store_name || '') + '</title>' +
      ...
      'th,td{border:1px solid #333;padding:8px;text-align:left}h2{font-family:Fraunces,serif}</style></head><body>' +
      '<h2>Nota Pembayaran ' + escapeHtml(settings.store_name || '') + '</h2>' +
